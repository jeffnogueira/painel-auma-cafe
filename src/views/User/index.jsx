...
class Index extends React.Component {
    ...
    deleteUser = async idUser => {
        try {
            await axios.delete(`${server}/user/${idUser}`)
            await this.loadUser()
        } catch (err) {
            showError(err)
        }
    ...
    render(){
        ...
        const pageNumbers = []
        for (let i = 1; i <= Math.ceil(this.state.users.length / this.state.allPerPage); i++) {
            pageNumbers.push(i)
        }
        ...
        const allUsers = currentAll.map(user => {
            return ([ user.description, user.name, user.email, 
                      <div>
                          <Button value="Editar" color="info" onClick={this.setRedirect.bind(this, `editUser/${user.id}`)}>Editar</Button>
                          <Button value="Excluir" onClick={this.confirmDelete.bind(this, user.id)} color="danger">Excluir</Button>
                      </div>
            ])
        })
                    ...
                    <GridItem xs={12} sm={12} md={12}>
                        <Card>
                            ...
                            <div style={{marginTop: 5,marginLeft: 15,marginRight: 15}}>
                                {this.renderRedirect()}
                                <Button value="Novo" style={{float:'right'}} color="info" onClick={this.setRedirect.bind(this, 'addUser')}>Novo</Button>
                            </div>
                            ...
                            <div>
                                <ul style={{listStyle: 'none',display: 'flex'}}>
                                    ...
                                    {renderPageNumbers}
                                    <li className={classes.itemNumber}
                                        key={Math.ceil(this.state.users.length / this.state.allPerPage)}
                                        id={Math.ceil(this.state.users.length / this.state.allPerPage)}
                                        onClick={this.handleClick}
                                    >
                                        {'>>'}
                                    </li>
